...
const Music = () => {
  ...
  const [activeVideo, setActiveVideo] = useState(null); // ✅ FIX ADDED
  const navigate = useNavigate();

  useEffect(() => {
    fetch("https://healthmate-evbq.onrender.com/mental/search?q=peaceful%20music")
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <section
      ...
      style={{
        background: `linear-gradient(to bottom, #3D202E 0%, #80361F 100%)`,
      }}
    >
      <div className="relative max-w-7xl mx-auto text-left">
        <div className="rounded-xl min-h-[500px] bg-[#ffffff0d] backdrop-blur-sm p-8 shadow-2xl shadow-black/50">
          ...
          <p className="text-white text-sm md:text-base mb-12 opacity-90">
            Relaxing, mindful music curated to calm your mind.
          </p>
