...
const Service = forwardRef((props, ref) => {
  ...
  const services = [
    ...
    },
    {
      ...
      href: "https://healthmate-bot.streamlit.app/",
    },
    {
  ...
  const handleCardClick = (index) => {
    ...
    if (selected.href) {
      window.open(selected.href, "_blank"); // open AI Health Assistant externally
    } else if (selected.path) {
      navigate(selected.path); // navigate internal routes
    }
  };
    ...
    <section ref={ref} className="px-8 py-20 bg-white ml-56 mr-56 mt-8">
      <div className="max-w-7xl mx-auto">
        <div className="text-start mb-24 ">
          ...
          <h2 className="text-4xl font-bold text-black mt-4 mb-4" style={{ letterSpacing: "0.2px" }}>
            See Our Impressions
          </h2>
          ...
          {services.slice(0, 4).map((service, index) => (
            ...
            >
              <div className="ml-8 mb-6 mt-8 flex justify-start">
                {React.cloneElement(service.icon, { className: `w-16 h-16 ${activeCard === index ? 'text-white' : service.icon.props.className}` })}
              </div>
              <h3 className={`ml-8 text-xl text-left font-bold mb-4 ${activeCard === index ? 'text-white' : 'text-gray-900'}`}>
                {service.title}
              </h3>
              <div className="ml-8 w-[50px] h-[2px] mb-4 bg-[#E74040]"></div>
              <p className={`ml-8 text-sm text-left leading-relaxed ${activeCard === index ? 'text-white/90' : 'text-gray-600'}`}>
                {service.description}
              </p>
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 ml-30 mr-30 mb-8">
          {services.slice(4, 7).map((service, index) => (
            <div
              ...
              style={{ background: activeCard === (index + 4) ? 'linear-gradient(135deg, #FF6B3D 0%, #8A5EA8 50%)' : '#ffffff' }}
              onMouseEnter={(e) => {
                e.currentTarget.style.background = 'linear-gradient(to bottom, #FF6B3D 0%, #8A5EA8 100%)';
                e.currentTarget.style.color = 'white';
              }}
            ...
            >
              <div className="ml-8 mb-6 mt-8 flex justify-start">
                {React.cloneElement(service.icon, { className: `w-16 h-16 ${activeCard === (index + 4) ? 'text-white' : service.icon.props.className}` })}
              </div>
              <h3 className={`ml-8 text-xl text-left font-bold mb-4 ${activeCard === (index + 4) ? 'text-white' : 'text-gray-900'}`}>
                {service.title}
              </h3>
              <div className="ml-8 w-[50px] h-[2px] mb-4 bg-[#E74040]"></div>
              <p className={`ml-8 text-sm text-left leading-relaxed ${activeCard === (index + 4) ? 'text-white/90' : 'text-gray-600'}`}>
                {service.description}
              </p>
            </div>
          ))}
